# Assignment Dataset Generation Process

Audience: reviewers evaluating the resource allocator assignment dataset.
...
member persona Marcus Tan. It is designed to show that the scheduler can:
...
Prompt files live under `prompts/` and are source material for dataset
generation. They are not read by the scheduler or web app at runtime; runtime
behavior uses the accepted JSON and Markdown artifacts under `data/` and
`data/runs/demo-run/`.

The prompt directory is organized by role:

- `prompts/fragments/` contains reusable rules shared across stages, including
  the system role, client profile rules, realism rules, resource rules,
  availability rules, activity schema rules, and batch instructions.
- `prompts/stages/` contains stage-specific task instructions for generating
  each dataset artifact.
- `prompts/inputs/` contains raw source material, currently the original persona
  seed used by Stage 01.
- `prompts/examples/` contains one-shot structural examples used to guide output
  shape without copying the example content.
- `prompts/assembled/` contains full prompts generated by
  `scripts/assemble_prompts.py`. These combine reusable fragments, stage
  instructions, current `data/*.json` inputs, and Stage 05 batch context into
  prompts that can be sent to OpenAI.
- `prompts/repair.prompt.md` contains generic repair instructions for invalid
  generated artifacts. It is intended to preserve valid generated content while
  fixing validation failures.

The automated OpenAI regeneration scripts currently consume assembled prompts
for Stage 05A and Stage 05B. `scripts/generate_stage05_blueprint.py` reads the
assembled blueprint prompt, validates the model response, and can accept it into
`data/activity_family_blueprint.json`. `scripts/generate_stage05_activity_batches.py`
reads assembled batch prompts, validates each generated batch, stores local
generation intermediates under `data/generated/`, and merges accepted batches
into `data/activity_families.json` and `data/action_plan.json`.
...
Consultation scheduling was raised for high-touch member-support realism. Clinical and
